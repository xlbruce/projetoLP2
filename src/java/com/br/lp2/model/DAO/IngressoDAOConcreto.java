...
public class IngressoDAOConcreto implements IngressoDAO {
    ...
    public boolean insertIngresso(Ingresso d) {
        ...
        try {
            ...
            ps.setString(3, d.getTipo().toString());
            if (ps.execute()) {
                return true;
            }
    ...
    public ArrayList<Ingresso> readIngressos() {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                ingressos.add(new Ingresso(rs.getInt("pk"), rs.getInt("numero"),
                        rs.getBoolean("inteira"),
                        (Cliente.Especiais.valueOf(rs.getString("tipo")))));
            }
        } catch (SQLException ex) {
            System.out.println(ex.getMessage());
        }
    ...
    public Ingresso readIngressoByInteira(Boolean inteira) {
        ...
        try {
            ...
            return new Ingresso(rs.getInt("pk"), rs.getInt("numero"),
                    rs.getBoolean("inteira"), (Cliente.Especiais) rs.getObject("tipo"));
        } catch (Exception e) {
